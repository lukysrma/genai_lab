...
def generate_response(query, index):
    if not index:
        return (
            "I'm sorry, I cannot process the IPC without the index. "
            "Please ensure the IPC content is loaded."
        )
    ...
    if not section_text:
        return (
            "I found the relevant section, but I'm unable "
    ...
    response = (
        f"Here's what I found in the Indian Penal Code, "
        f"**{relevant_section_title}**:\n\n{cleaned_text}"
    )
...
def chatbot(index):
    print(
        "Welcome to the Indian Penal Code Chatbot! "
        "Ask me anything about the IPC. Type 'exit' to quit."
    )
